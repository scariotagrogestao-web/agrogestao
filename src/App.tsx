...
export default function App() {
  ...
  const isCurrentUserAdmin = useMemo(() => {
    if (currentUser === 'admin' || currentUser === 'anderson') return true;
    const found = customUsers.find(u => u.username.toLowerCase() === currentUser?.toLowerCase());
  ...
  const canCurrentUserDelete = useMemo(() => {
    ...
    if (found.canDelete === false) return false;
    return found.role === 'admin' ? (found.canDelete !== false) : false;
  }, [currentUser, customUsers]);

  const checkCanDeleteOrDeny = (entityName: string): boolean => {
    if (!canCurrentUserDelete) {
      addAuditLog('Segurança (Bloqueio Exclusão)', `Tentativa de exclusão negada em ${entityName} pelo usuário ${currentUser}`);
      alert('Acesso Negado (403 Forbidden): Você não possui permissão para excluir registros no AgroGestão.');
      return false;
    }
    return true;
  };
  ...
  const handleDeleteClientOrVehicle = (id: string) => {
    if (!checkCanDeleteOrDeny('Veículos/Clientes')) return;
  ...
  const handleAddArea = (item: Area) => {
    ...
    addAuditLog('Cadastro', `Adicionou área: ${item.name}`);
  };
  const handleEditArea = (updated: Area) => {
    setAreas(prev => prev.map(a => a.id === updated.id ? updated : a));
    addAuditLog('Cadastro', `Alterou área: ${updated.name}`);
  };
  const handleDeleteArea = (id: string) => {
    if (!checkCanDeleteOrDeny('Áreas/Fazendas')) return;
    const item = areas.find(a => a.id === id);
    setAreas(prev => prev.filter(a => a.id !== id));
    addAuditLog('Cadastro', `Removeu área: ${item?.name || id}`);
  };

  const handleAddMotorista = (item: Motorista) => {
    setMotoristas(prev => [...prev, item]);
    addAuditLog('Cadastro', `Adicionou motorista: ${item.name}`);
  };
  const handleEditMotorista = (updated: Motorista) => {
    setMotoristas(prev => prev.map(m => m.id === updated.id ? updated : m));
    addAuditLog('Cadastro', `Alterou motorista: ${updated.name}`);
  };
  const handleDeleteMotorista = (id: string) => {
    if (!checkCanDeleteOrDeny('Motoristas/Operadores')) return;
    const item = motoristas.find(m => m.id === id);
    setMotoristas(prev => prev.filter(m => m.id !== id));
    addAuditLog('Cadastro', `Removeu motorista: ${item?.name || id}`);
  };
  ...
  const handleDeleteMachine = (sheetId: string, machineId: string) => {
    if (!checkCanDeleteOrDeny('Máquinas')) return;
    setLocalitySheets(prev => prev.map(s => {
      if (s.id !== sheetId) return s;
      const machine = s.machines.find(m => m.id === machineId);
      if (machine) {
  ...
  const handleDeleteLocality = (sheetId: string) => {
    if (!checkCanDeleteOrDeny('Localidades')) return;
  ...
  const handleDeleteExpense = (id: string) => {
    if (!checkCanDeleteOrDeny('Despesas')) return;
  ...
  const handleDeleteProducao = (id: string) => {
    if (!checkCanDeleteOrDeny('Produção')) return;
