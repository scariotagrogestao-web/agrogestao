...
/**
 * Calculador unificado de Receita proveniente exclusivamente da Produção de Silagem.
 * A receita vem de valorTotalReceita ou (toneladas * precoPorTon).
 */
export const calculateSilagemRevenue = (producoes: Producao[] = []): number => {
  return producoes.reduce((sum, p) => {
...
/**
 * Calculador de custo total de pagamentos a motoristas/operadores (Hectares x Taxa do Motorista).
 */
export const calculateDriverCosts = (producoes: Producao[] = [], motoristas: Motorista[] = []): number => {
  return producoes.reduce((sum, p) => {
    const motorista = motoristas.find(m => m.id === p.motoristaId);
    const rate = motorista?.ratePerHectare || 0;
    return sum + (p.hectares * rate);
  }, 0);
};
