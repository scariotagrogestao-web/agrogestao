...
export default function ProducaoEntryView({
  ...
  const [precoPorTon, setPrecoPorTon] = useState('');
  const [valorTotalReceita, setValorTotalReceita] = useState('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const pTonValue = precoPorTon ? parseFloat(precoPorTon) : undefined;
    const revTotalValue = valorTotalReceita ? parseFloat(valorTotalReceita) : (pTonValue && tonValue ? pTonValue * tonValue : undefined);
    ...
    if (editingId) {
      onEditProducao({
        ...
        hectares: haValue,
        toneladas: tonValue,
        precoPorTon: pTonValue,
        valorTotalReceita: revTotalValue
      });
      setEditingId(null);
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const tVal = parseFloat(toneladas);
                    if (!isNaN(pVal) && !isNaN(tVal)) {
                      setValorTotalReceita((pVal * tVal).toFixed(2));
                    }
                  }}
              ...
              <div>
                ...
                <input
                  type="number"
                  step="0.01"
                  value={valorTotalReceita}
                  onChange={(e) => setValorTotalReceita(e.target.value)}
                  placeholder="Ex: 18060.00"
                  className="w-full border border-slate-200 rounded-lg p-2.5 text-xs text-slate-800 outline-none focus:ring-1 focus:ring-emerald-600 bg-slate-50"
                />
              </div>
            </div>
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            onClick={() => {
                              ...
                              setPrecoPorTon(prod.precoPorTon ? String(prod.precoPorTon) : '');
                              setValorTotalReceita(prod.valorTotalReceita ? String(prod.valorTotalReceita) : '');
                            }}
