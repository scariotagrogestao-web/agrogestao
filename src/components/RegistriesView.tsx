...
export default function RegistriesView({
  ...
  const openEditModal = (item: any) => {
    ...
    if (activeSubTab === 'vehicles') {
      ...
      setAName(a.name || '');
      setACulture(a.culture || 'Silagem');
      setASize(a.sizeHectares !== undefined && a.sizeHectares !== null ? a.sizeHectares.toString() : '');
    } else if (activeSubTab === 'drivers') {
      const d = item as Motorista;
      setDName(d.name || '');
      setDRate(d.ratePerHectare !== undefined && d.ratePerHectare !== null ? d.ratePerHectare.toString() : '60');
      setDStatus(d.status ? (d.status.toLowerCase() === 'inativo' ? 'Inativo' : 'Ativo') : 'Ativo');
    }
                ...
                {filteredAreas.map((a, idx) => {
                  ...
                  return (
                    <tr key={a.id} style={{ backgroundColor: bg }} className="hover:opacity-95 transition-opacity">
                      ...
                      <td className="py-3 px-4 text-right font-mono font-bold text-slate-800">
                        {(a.sizeHectares ?? 0).toLocaleString('pt-BR')} ha
                      </td>
                      <td className="py-3 px-4 text-center">
                        <div className="flex gap-1.5 justify-center">
                          <button
                ...
                {filteredDrivers.map((d, idx) => {
                  ...
                  return (
                    <tr key={d.id} style={{ backgroundColor: bg }} className="hover:opacity-95 transition-opacity">
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className={`badge ${d.status?.toLowerCase() === 'ativo' ? 'badge-green' : 'badge-gray'}`}>
                          {d.status || 'Ativo'}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-right font-mono font-bold text-emerald-800">
                        R$ {(d.ratePerHectare ?? 60).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}/ha
                      </td>
                      <td className="py-3 px-4 text-center">
                        <div className="flex gap-1.5 justify-center">
                          <button
