...
export default function ExpensesView({ 
  ...
  // Month selector state: YYYY-MM (default: current month)
  ...
  const handlePrevMonth = () => {
    if (selectedYearMonth === 'all') return;
  ...
  // Derived month dates
  const { monthStartDate, monthEndDate, selectedMonthLabel } = useMemo(() => {
    if (selectedYearMonth === 'all') {
      return { monthStartDate: '', monthEndDate: '', selectedMonthLabel: 'Todo o Histórico' };
    }
    const [selYearStr, selMonthStr] = selectedYearMonth.split('-');
    const selMonthIndex = parseInt(selMonthStr, 10) - 1;
    const selectedMonthName = monthNamesPt[selMonthIndex] || 'Agosto';
    const selectedYear = parseInt(selYearStr, 10);
    const daysInSelMonth = new Date(selectedYear, selMonthIndex + 1, 0).getDate();

    return {
      monthStartDate: `${selectedYearMonth}-01`,
      monthEndDate: `${selectedYearMonth}-${String(daysInSelMonth).padStart(2, '0')}`,
      selectedMonthLabel: `${selectedMonthName} / ${selectedYear}`
    };
  }, [selectedYearMonth]);
  ...
  const filteredExpenses = useMemo(() => {
    const list = expenses.filter(exp => {
      ...
      const matchesMonth = selectedYearMonth === 'all' || 
        (exp.date >= monthStartDate && exp.date <= monthEndDate);
      ...
      return matchesMonth && matchesType && matchesMachine && matchesDriver && matchesArea && matchesDate && matchesSearch;
    });
  ...
  }, [expenses, selectedYearMonth, monthStartDate, monthEndDate, typeFilter, machineFilter, driverFilter, areaFilter, sortOrder, startDate, endDate, searchQuery]);
        ...
        <div className="col-span-1 lg:col-span-8 flex flex-col gap-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 shrink-0">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Despesa Total ({selectedMonthLabel})</p>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Maior Gasto no Mês</p>
